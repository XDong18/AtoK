...
class BasicDataset(Dataset):
    ...
    def preprocess_image(cls, img):
        ...
        # img = cv2.resize(img, (448, 320), interpolation=cv2.INTER_NEAREST)
        ...
        for point in point_list:
            cv2.circle(img, (point[1], point[0]), 12, 
            (int(img[point[0], point[1], 0]), int(img[point[0], point[1], 1]), int(img[point[0], point[1], 2])), -1)
        
        gray_img = cv2.resize(img[:, :, 0], (448, 320), interpolation=cv2.INTER_NEAREST)
        img_new = np.array([gray_img])
    ...
    @classmethod
    def preprocess_mask(cls, img, ignore_mask, dense_mask):
        img = cv2.resize(img, (1920, 1216))
        # img = cv2.resize(img, (448, 320), interpolation=cv2.INTER_NEAREST)
        # noise = noise * boxes
        ...
        new_img = cv2.resize(new_img, (448, 320), interpolation=cv2.INTER_NEAREST)
...
if __name__=='__main__':
    ...
    npy_img = join('/shared/xudongliu/data/argoverse-tracking/argo_track/' + data_split, 'npy_img_dense_resize')
    npy_mask = join('/shared/xudongliu/data/argoverse-tracking/argo_track/' + data_split, 'npy_mask_dense_resize')
